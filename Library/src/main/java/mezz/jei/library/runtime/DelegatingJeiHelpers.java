...
public class DelegatingJeiHelpers implements IJeiHelpers {
	...
	private final CompletableFuture<IJeiHelpers> delegateFuture = new CompletableFuture<>();
	...
	private IJeiHelpers blockingGetDelegate() {
		...
		try {
			return delegateFuture.get(30, TimeUnit.SECONDS);
		} catch (InterruptedException | ExecutionException | TimeoutException e) {
			throw new RuntimeException("JEI helpers not available yet", e);
		}
	}
	...
	public ICodecHelper getCodecHelper() {
		return blockingGetDelegate().getCodecHelper();
	}
}
